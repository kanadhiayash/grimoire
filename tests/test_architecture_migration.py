...
class ArchitectureMigrationTests(unittest.TestCase):
    def test_architecture_records_are_normative_source_linked_contracts(self):
        records = load_standard_registry(ROOT / "registry" / "standards", root=ROOT)
        selected = [r for r in records.values() if r.domain == "architecture"]
        self.assertEqual(len(selected), 2)
        self.assertTrue(all(r.provenance["source_id"].startswith("SRC-ARCH-") for r in selected))
