...
class FrontendMigrationTests(unittest.TestCase):
    def test_frontend_record_is_normative_source_linked_contract(self):
        records = load_standard_registry(ROOT / "registry" / "standards", root=ROOT)
        selected = [r for r in records.values() if r.domain == "frontend"]
        self.assertEqual(len(selected), 1)
        self.assertEqual(selected[0].provenance["source_id"], "SRC-FRONTEND-CLIENT")
