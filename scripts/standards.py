#!/usr/bin/env python3
"""Unified human and agent command surface for Engineering Standards."""
...
def build_status(root: Path = ROOT) -> dict[str, Any]:
    ...
    return {
        ...
        "surface_count": len(index.get("surfaces", {}))
        if isinstance(index.get("surfaces"), dict)
        else 0,
        "command_count": len(index.get("commands", {}))
        if isinstance(index.get("commands"), dict)
        else 0,
...
def run_steps(
    steps: Sequence[tuple[str, Sequence[str]]], root: Path = ROOT
) -> list[dict[str, Any]]:
...
def build_pack_command(
    *,
    ...
    project_name: str,
    output: str,
    engineering_standards_commit: str,
    zeref_commit: str,
    overwrite: bool = False,
) -> list[str]:
    command = [
        sys.executable,
        "scripts/compile_surface_pack.py",
        "--surface",
        surface,
        "--project-name",
        project_name,
...
def parser() -> argparse.ArgumentParser:
    ...
    harness = subparsers.add_parser(
        "harness", help="Manage global harness adapters"
    )
    harness.add_argument(
        "action",
        choices=["detect", "plan", "apply", "verify", "uninstall"],
    )
...
def main() -> int:
    ...
    if args.command == "pack":
        if args.pack_action == "compile":
            ...
            command = [
                sys.executable,
                "scripts/verify_surface_pack.py",
                args.directory,
            ]
